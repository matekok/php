...
{
    $yahooCSV = "http://finance.yahoo.com/d/quotes.csv?e=.csv&f=sl1d1t1&s=" . $symbol . "=X";
...
function dlFile($url)
{
    ...
    $crl = curl_init();
    curl_setopt($crl, CURLOPT_PROXY, $proxy[$i]['ip'] . ':' . $proxy[$i]['port']);
...
function getYahooData($array)
{
    ...
    //$currencys .= $array[0] . '=X,';
    $currencys = substr($currencys, 0, -1);
    //finance.yahoo.com/d/quotes.csv?e=.csv&f=sl1d1t1&s=eurUSD=X,usdGBP=X,hufEUR=X
    $yahooCSV = 'http://finance.yahoo.com/d/quotes.csv?e=.csv&f=sl1d1t1&s=' . $currencys;
    print $yahooCSV;
    //$csv = getDataWithWPN($yahooCSV);
    $csv = fopen($yahooCSV, 'r');
    ...
    for ($i = 0; $row = fgetcsv($csv); ++$i) {
        //print_r($row);
        list($quote['symbol'], $quote['last'], $quote['date'], $quote['timestamp'], $quote['change'], $quote['open'],
            $quote['high'], $quote['low'], $quote['volume'], $quote['previousClose'], $quote['name'], $quote['bid'],
            $quote['ask'], $quote['eps'], $quote['YearLow'], $quote['YearHigh'], $quote['PE']) = $row;
...
function getRate($dataArray = array(), $cur1, $cur2)
{
    $array = array();
    foreach ($dataArray as $key => $value) {
        if (strtolower($key) == strtolower($cur1) . strtolower($cur2)) { //ha j�l van a form�tum
            $array['date'] = $value['date'];
            $array['rate'] = $value['rate'];
            return $array;
        } elseif (strtolower($key) == strtolower($cur2) . strtolower($cur1)) { // ha ford�tva van
            $array['date'] = $value['date'];
            $array['rate'] = 1 / $value['rate'];
            return $array;
        }
    }
    return array('rate' => '', 'date' => '');
}
...
function XML2Array($xml)
{
    ...
    foreach ($xmlObject->children() as $node) {
        ...
        $name = (string)$arr["Symbol"];
        $backData = array();
        foreach ($node as $key => $value) {
            $backData[$key] = (string)$value;
        }
        $backData['rate'] = (string)$node->Ask;
        $backData['date'] = (string)$node->Last;

        $clock = strtotime('+5 hours', strtotime($backData['date']));
        $date = strtotime('-1 hours', strtotime(date('Y-m-d H:i:s')));
        $backData['date'] = date('Y-m-d h:i:s', strtotime(date('Y-m-d', $date) . ' ' . date('H:i:s', $clock)));
        $array[strtolower($name)] = $backData;
    }
    return $array;
}

function addToCourency()
{
    global $data, $data2, $defCur;
    $url = 'http://www.floatrates.com/daily/' . strtolower($defCur) . '.json';
    $url = getDataWithVPN($url);
    $data3 = json_decode($url, TRUE);
    foreach ($data3 as $key => $value) {
        if (empty($data2[$value['code']])) {
            $data2[$defCur . $value['code']] = array('rate' => $value['rate'], 'date' => date('Y-m-d h:i:s', strtotime($value['date'])));
        }
    }
    /*    foreach ($data as $key => $value) {
      if(empty($data2[strtolower($value['code'].$defCur)]) && empty($data2[strtolower($defCur.$value['code'])])){
      $symbol = $value['code'].$defCur;
      $data3 = getQuote($symbol);
      print_r($data3);print '<br><br>';
      }
      } */
// print_r($data2);
    return $data2;
...
{
    //$stm = "INSERT INTO Friends VALUES(1,'Jane', 'F')";
    $dbhandle = sqlite_open('db/test.db', 0666, $error);
    if (!$dbhandle) die ($error);
    $ok1 = sqlite_exec($dbhandle, $script);
    if (!$ok1) die("Cannot execute statement.");
    echo "Data inserted successfully";
    sqlite_close($dbhandle);
}

function readSQLite($database, $query)
{
...
{
    $userAgent = array();
    $userAgent['InternetExplorer'] = array(
    ...
    $browser = array('Chrome', 'Firefox', 'InternetExplorer', 'opera', 'safari');
    $key = array_rand($userAgent);

    return $userAgent[$key][array_rand($userAgent[$key])];
}

function getVpnArray()
{
    global $user_agent;
...
//    print $html;
    ...
    $node = $finder->query("//*[contains(@class, 'proxy-table')]");
...
//    print $html;
    ...
    for ($i = 0; $i < count($arr);) {
        ...
        $array1['port'] = $arr[$i];
        $i++;
        $array1['country'] = trim($arr[$i]);
        $i++;
        $array1['speed'] = trim($arr[$i]);
        $i++;
        $array1['type'] = $arr[$i];
        $i++;
        $array1['anonimity'] = $arr[$i];
        $i++;
        $array1['age'] = $arr[$i];
        $i++;
        $array[] = $array1;
...
//    $proxy = getVpnArray();
    $i = 0;
    while ($result == '') {
        $PROXY_HOST = $proxy[$i]['ip']; // Proxy server address
        $PROXY_PORT = $proxy[$i]['port'];    // Proxy server port
        $PROXY_USER = "";    // Username
        $PROXY_PASS = "";   // Password
...
$tax = 0.05;
$defCur = 'USD';
//$proxy = getVpnArray(); //egyelőre nem kell a Proxy lista úgysem megy
...
foreach ($data as $key => $value) {
    ...
    $rate = getRate(array(), $defCur, $value['code']);
    $CurPrice = 0;
    $CurDate = '1900-01-01';
    if (!empty($rate['rate'])) $CurPrice = $rate['rate'];
    if (!empty($rate['date'])) $CurDate = $rate['date'];

    $price[$value['code']] = array();
    $price[$value['code']]['name'] = $value['name'];
    $price[$value['code']]['sale'] = round(($value['rate'] * (1 + $tax)), 2);
    $price[$value['code']]['price'] = round($value['rate'], 2);
    $price[$value['code']]['buy'] = round(($value['rate'] * (1 - $tax)), 2);
    $price[$value['code']][$defCur . '-' . $value['code'] . '-middle'] = round($CurPrice, 2);
    $price[$value['code']][$defCur . '-' . $value['code'] . '-buy'] = round($CurPrice * (1 - $tax), 2);
    $price[$value['code']][$defCur . '-' . $value['code'] . '-sale'] = round($CurPrice * (1 + $tax), 2);
    $price[$value['code']][$defCur . 'date'] = $CurDate;
//    }
    //bitconinn�l nincs v�lt�si d�j
    if ($value['code'] == 'BTC') {
        $price[$value['code']] = array();
        $price[$value['code']]['name'] = $value['name'];
        $price[$value['code']]['sale'] = 0;
        $price[$value['code']]['price'] = 1;
        $price[$value['code']]['buy'] = 0;
        $price[$value['code']][$defCur . '-' . $value['code'] . '-middle'] = round($CurPrice, 2);
        $price[$value['code']][$defCur . '-' . $value['code'] . '-buy'] = round($CurPrice * (1 - $tax), 2);
        $price[$value['code']][$defCur . '-' . $value['code'] . '-sale'] = round($CurPrice * (1 + $tax), 2);
        $price[$value['code']][$defCur . 'date'] = $CurDate;
...
print '<html><head><!-- Chrome, Firefox OS and Opera -->
<meta name="theme-color" content="#4285f4">
<!-- Windows Phone -->
<meta name="msapplication-navbutton-color" content="#4285f4">
<!-- iOS Safari -->
<meta name="apple-mobile-web-app-status-bar-style" content="#4285f4"></head><body>';
print '<table>';
print '<tr><td>sort name</td><td>name</td><td>BTC sale</td><td>BTC middle</td><td>BTC buy</td><td>' . $defCur . ' sale</td><td>' . $defCur . ' middle</td><td>' . $defCur . ' buy</td></tr>';
foreach ($price as $key => $value) {
    print '<tr><td><!--<a href="https://www.easymarkets.com/api/jsapiservice.svc/?rq=%5B%7B%22action%22%3A%22GetMarketInfoRates%22%2C%22args%22%3A%7B%22fetchOnlyRequestedCps%22%3Atrue%2C%22symbolsSet%22%3A%5B%22USD' . $key . '%22%5D%7D%7D%5D&sid=3675_89f95ccf-3cbe-47fd-872f-767a408ffd1f&timestamp=' . time() . '&appid=CA7D0F97-F865-4D89-9983-409E5EE5DDF3" target="_blank">-->' . $key . '<!--</a>--></td><td>' . $value['name'] . '</td><td align="right">' . number_format($value['sale'], 2, ',', ' ') . '</td><td align="right">' . number_format($value['price'], 2, ',', ' ') . '</td><td align="right">' . number_format($value['buy'], 2, ',', ' ') . '</td><td align="right">' . number_format($value[$defCur . '-' . $key . '-sale'], 2, ',', ' ') . '</td><td align="right">' . number_format($value[$defCur . '-' . $key . '-middle'], 2, ',', ' ') . '</td><td align="right">' . number_format($value[$defCur . '-' . $key . '-buy'], 2, ',', ' ') . '</td><td align="right">' . $value[$defCur . 'date'] . '</td></tr>';
}
print '</table>';
print '</body></html>';
